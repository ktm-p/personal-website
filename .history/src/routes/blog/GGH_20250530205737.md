...
To begin with, we note that we will be communicating in a language like English. However, GGH represent messages using integer vectorss; we need a way to map characters to integers! A natural approach then is to use <a href="https://en.wikipedia.org/wiki/UTF-8" target="_blank">UTF-8 encoding <sup><i class='fas fa-external-link-alt arrow'></i></sup></a>.
```python
