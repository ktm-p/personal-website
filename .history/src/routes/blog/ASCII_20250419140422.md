...
The question then is how we can exactly represent all of these colors using a set of $16$ colors only. An initial thought may be to try what we did for the brightness by constructing ``buckets" for each of the possible colors.

However, due to colors being composed of three separate values for <span style="color: red">Red</span>/Green/Blue