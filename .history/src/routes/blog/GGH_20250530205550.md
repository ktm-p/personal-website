...
Before implementing the encryption scheme, we will need to construct some helper functions first. To begin with, we note that since GGH deals with integer vectors, whereas we want to communicate in a language like English, we need some way to convert a character into integers. A natural approach then is to use <a href="https://en.wikipedia.org/wiki/UTF-8" target="_blank">UTF-8 encoding <sup><i class='fas fa-external-link-alt arrow'></i></sup></a>:
```python
