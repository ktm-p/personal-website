...
A natural choice of metric for determining closeness would be the Euclidean distance between two colors. So, we want to calculate the following:
$$
\begin{equation*}
    c^* = \mathrm{argmin}_\limits{c'} \sqrt{(c - c')^2}
\end{equation*}
$$
