...
## Implementation
Now, with that in mind, we can begin coding this up in Python. First, we will define a function `grayscale()` that takes in an iamge and grayscale it. This can be done simply using the `PIL` library:
...
Now, we want to actually extract our image given an image path, and then grayscale it using the `grayscale()` function we wrote:
```python
def main():
