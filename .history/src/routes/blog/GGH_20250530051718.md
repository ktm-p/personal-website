...
Now, we have all of the ingredients for constructing the GGH Encryption Scheme. First, we need to decide on the dimension $N$ of our lattice and the ``noise parameter" $\sigma$. Both of these values are pre-determined and public.

Next, since GGH is an asymmetric encryption method, we need a private and public key. 
